...
function verifyparallel {
    ...
    echo "$VERIFYPN -n -q $TIMEOUT_SIMP -l $TIMEOUT_LP -d $TIMEOUT_RED -z 4 -s OverApprox --write-simplified $QF --write-reduced $MF -x $MULTIQUERY_INPUT $MODEL_PATH/model.pnml $CATEGORY"

    TMP=$($TIMEOUT_CMD $SECONDS $VERIFYPN -n -q $TIMEOUT_SIMP -l $TIMEOUT_LP -d $TIMEOUT_RED -z 4 -s OverApprox --binary-query-io 2 --write-simplified $QF --write-reduced $MF -x $MULTIQUERY_INPUT $MODEL_PATH/model.pnml $CATEGORY)
...
case "$BK_EXAMINATION" in
   ...
   StableMarking)
    ...
    OneSafe)
        echo        
        echo "**********************************************"
        echo "*  TAPAAL checking for OneSafe              *"
        echo "**********************************************"
	TIMEOUT_SEQ_MIN=0 
        CATEGORY="${DIR}/${BK_EXAMINATION}.xml"
	TIMEOUT_PAR=$(echo "$TIMEOUT_TOTAL/6" | bc) # competition 10 min
