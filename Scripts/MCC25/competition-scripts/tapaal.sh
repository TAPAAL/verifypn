...
TIMEOUT_EX=$(echo "$TIMEOUT_TOTAL/30" | bc)  # for explicit CPN engine
...
function verifyparallel {
    ...
    # Reindex the array
    QUERIES=($(for q in ${QUERIES[@]}; do echo $q; done )) 
    ...
    # Step -0.5: Explicit   (only for colored nets and reachability)
    colored=`cat $MODEL_PATH/iscolored`
    if [ "$colored" = "TRUE" ]; then
        if [ "$BK_EXAMINATION" = "ReachabilityCardinality" ] || [ "$BK_EXAMINATION" = "ReachabilityFireability" ]; then

        echo "---------------------------------------------------"
        echo "            Step -0.5: Explicit parallel processing"
        echo "---------------------------------------------------"
        echo "Doing explicit parallel verification of individual queries ($NUMBER in total)"
        echo "Each query is verified by ${#STRATEGIES_EX[@]} parallel strategies for $TIMEOUT_EX seconds"

        i=0
        for Q in ${QUERIES[@]}; do

            TIMEOUT_EX=$(( $TIMEOUT_EX < $SECONDS ? $TIMEOUT_EX : $SECONDS))
            if [[ "$TIMEOUT_EX" -le 0 ]] ; then echo "Out of time, terminating!"; time_left; exit; fi
            echo "------------------- QUERY ${Q} ----------------------"
            # Execute verifypn on all parallel strategies
            # All processes are killed if one process provides an answer 
            step1="$($PAR_CMD --line-buffer --halt now,success=1 --timeout $TIMEOUT_EX --xapply\
                eval $TIME_CMD $VERIFYPN $OPTIONS {} $MODEL_PATH/model.pnml $CATEGORY  -x $Q \
                ::: "${STRATEGIES_EX[@]}" 2>&1)"
	        ...
	        NUMBER=$(echo "$NUMBER-1" | bc)
                i=$(echo "$i - 1" | bc)
                echo "Solution found by explicit exploration (step -0.5)"
            else
                echo "$step1"
                echo "No solution found (step -0.5)"
            fi
            echo 
            echo "$FORMULA_RESULT"
            time_left
        ...
        if [[ ${#QUERIES[@]} == 0 ]]; then echo "All queries are solved in step -0.5" ; time_left; exit; fi

        MULTIQUERY_INPUT=$(echo ${QUERIES[@]} | sed -e "s/ /,/g")
        time_left

        if [ -z "$MULTIQUERY_INPUT" ]; then echo "All queries are solved in step -0.5" ; time_left; exit; fi
      fi
    fi
