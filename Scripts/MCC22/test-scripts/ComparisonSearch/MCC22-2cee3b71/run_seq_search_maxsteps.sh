...
if [[ $USEREDUCTION == 0 ]] ; then
    OPTIONSREDUCTION="-q 0 -r 0"
else
    OPTIONSREDUCTION=""
fi
...
function verifysequential {
    ...
    echo "The search strategy is $SEARCH_STRATEGY with $MAXSTEPS max steps and reduction = $USEREDUCTION (reduction options=\"${OPTIONSREDUCTION}\")"
    ...
    for Q in ${QUERIES[@]}; do
        ...
        step2="$($TIMEOUT_CMD $TIMEOUT_SEQ $TIME_CMD \
                $VERIFYPN $MF $QF --binary-query-io 1 -x $Q -s $SEARCH_STRATEGY -n --max-steps $MAXSTEPS $OPTIONSREDUCTION 2>&1)"
        RETVAL=$?
