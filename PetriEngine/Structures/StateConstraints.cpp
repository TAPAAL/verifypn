...
namespace PetriEngine {
    namespace Structures {
        ...
        StateConstraints* StateConstraints::merge(const StateConstraints* c) const {
            StateConstraints* nc = new StateConstraints(nPlaces);
        ...
        StateConstraints* StateConstraints::requirePlaceMin(const PetriNet& net, uint32_t place, int32_t value) {
            StateConstraints* c = new StateConstraints(net);
            c->setPlaceMin(place, value);
            return c;
        }

        StateConstraints* StateConstraints::requirePlaceMax(const PetriNet& net, uint32_t place, int32_t value) {
            StateConstraints* c = new StateConstraints(net);
            c->setPlaceMax(place, value);
            return c;
        }

        StateConstraints* StateConstraints::requirePlaceEqual(const PetriNet& net, uint32_t place, int32_t value) {
            StateConstraints* c = new StateConstraints(net);
            c->setPlaceMax(place, value);
            c->setPlaceMin(place, value);
            return c;
...
//                set_col_name(lp, i + 1, const_cast<char*> (net.transitionNames()[i].c_str()));
            ...
            std::vector<REAL> row = std::vector<REAL>(net.numberOfTransitions() + 1);
            for (size_t p = 0; p < nPlaces; p++) {
                // Set row zero
                ...
                if (pcs[p].min == pcs[p].max &&
                    ...
                    add_constraint(lp, row.data(), EQ, target);
                } else {
                    // There's always a min, even zero is interesting
                    double target = pcs[p].min - m0[p];
                    add_constraint(lp, row.data(), GE, target);
                    if (pcs[p].max != CONSTRAINT_INFTY) {
                        double target = pcs[p].max - m0[p];
                        add_constraint(lp, row.data(), LE, target);
                    }
                }
            }
            ...
            while ((result == OPTIMAL || result == SUBOPTIMAL) && traplimit-- < 0) {
                ...
                set_add_rowmode(lp, TRUE);
                add_constraint(lp, row.data(), GE, 1);
                set_add_rowmode(lp, FALSE);
        ...
        BitField StateConstraints::maxTrap(const PetriNet& net,
                BitField places,
                const int32_t* resultMarking) const {
        ...
        bool StateConstraints::isMarked(const BitField& places, const int32_t* marking) const {
