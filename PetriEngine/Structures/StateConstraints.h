...
#define CONSTRAINT_INFTY 1<<20
        ...
        public:
            ...
            StateConstraints(uint32_t nPlaces) {
                this->nPlaces = nPlaces;
                pcs = std::vector<Constraint>(nPlaces);
                reset();
            }
            ...
            bool setPlaceMax(uint32_t place, int32_t max) {
            ...
            /** Set place maximum, returns false if this causes a conflict */
            bool setPlaceMin(uint32_t place, int32_t min) {
            ...
            bool isImpossible(const PetriNet& net, const int32_t* marking) const;
            /** Attempts to solve using lp_solve size of fire vector */
            int fireVectorSize(const PetriNet& net, const int32_t* marking) const;
            ...
            static StateConstraints* requirePlaceMin(const PetriNet& net, uint32_t place, int32_t value);
            static StateConstraints* requirePlaceMax(const PetriNet& net, uint32_t place, int32_t value);
            static StateConstraints* requirePlaceEqual(const PetriNet& net, uint32_t place, int32_t value);
        private:
            uint32_t nPlaces;
            std::vector<Constraint> pcs;
            std::vector<Constraint> vcs;

            BitField maxTrap(const PetriNet& net, BitField places, const int32_t* resultMarking) const;
            bool isInMaxTrap(const PetriNet& net, size_t place, const BitField& places, const int32_t* resultMarking) const;
            BitField minimalTrap(const PetriNet& net, const int32_t* marking, const int32_t* resultMarking) const;
            bool isMarked(const BitField& places, const int32_t* marking) const;
        };
