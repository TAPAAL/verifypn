...
ReachabilityResult HashUnderApproximation::reachable(const PetriNet &net,
			...
			if(net.fire(t, s->marking(), s->valuation(), ns->marking(), ns->valuation())){
				...
				if(result.second){
					...
					if(query->evaluate(PQL::EvaluationContext(ns->marking(), ns->valuation(), &net)))
						return ReachabilityResult(ReachabilityResult::Satisfied,
									  "A state satisfying the query was found", expanded, explored, discovered, enabledTransitionsCount, -1, ns->pathLength(), ns->trace());
					stack.back().t = t + 1;
	...
	return ReachabilityResult(ReachabilityResult::Unknown,
							"Could not disprove the existence of a state not satisfying the query.", expanded, explored, discovered, enabledTransitionsCount, -1);
}
