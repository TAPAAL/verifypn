...
/*
    class QueryValueVisitor final : public PQL::Visitor {
        public:
            static std::unique_ptr<CompiledGammaQueryExpression> eval(
                ...
                QueryValueVisitor visitor{placeNameIndices, transitionNameIndices, cpn};
                visit(visitor, expr);

                return std::move(visitor._compiled);
            }
        protected:
            explicit QueryValueVisitor(
                const std::unordered_map<std::string, uint32_t>& placeNameIndices,
                const std::unordered_map<std::string, uint32_t>& transitionNameIndices,
                const ColoredPetriNet& cpn
            )
                : _cpn(cpn), _placeNameIndices(placeNameIndices) { }

            void _accept(const PQL::LiteralExpr *element) override {
                _queryValue = QueryValue::fromCount(element->value());
            }
            ...
            void _accept(const PQL::NotCondition *element) override  { invalid(); }
            void _accept(const PQL::AndCondition *expr) override  { invalid(); }
            void _accept(const PQL::OrCondition *expr) override  { invalid(); }
            void _accept(const PQL::LessThanCondition *element) override { invalid(); }
            void _accept(const PQL::LessThanOrEqualCondition *element) override { invalid(); }
            void _accept(const PQL::EqualCondition *element) override { invalid(); }
            void _accept(const PQL::NotEqualCondition *element) override  { invalid(); }
            void _accept(const PQL::DeadlockCondition *element) override { invalid(); }
            void _accept(const PQL::FireableCondition *element) override { invalid(); }
            void _accept(const PQL::EFCondition *condition) override { invalid(); }
            void _accept(const PQL::AGCondition *condition) override { invalid(); }
