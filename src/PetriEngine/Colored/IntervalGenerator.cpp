...
namespace PetriEngine {
    ...
    std::vector<Colored::interval_t> IntervalGenerator::getIntervalsFromInterval(Colored::interval_t *interval, uint32_t varPosition, int32_t varModifier, std::vector<Colored::ColorType*> varColorTypes){
    ...
    void IntervalGenerator::getArcVarIntervals(Colored::intervalTuple_t& varIntervals, std::unordered_map<uint32_t, int32_t> modIndexMap, Colored::interval_t *interval, std::vector<Colored::ColorType*> varColorTypes){
    ...
    bool IntervalGenerator::getVarIntervals(std::vector<std::unordered_map<const Colored::Variable *, Colored::intervalTuple_t>>& variableMaps, std::unordered_map<uint32_t, Colored::ArcIntervals> placeArcIntervals){
            ...
            for(uint32_t j = 0; j < placeArcInterval.second._intervalTupleVec.size(); j++){
                ...
                if(variableMaps.empty()){                    
                    ...
                    for(uint32_t i = 0; i < intervalTupleSize; i++){
                        ...
                        for(auto pair : placeArcInterval.second._varIndexModMap){
                            ...
                            if(placeArcInterval.second._intervalTupleVec.size() > 1 && pair.second[j].empty()){
