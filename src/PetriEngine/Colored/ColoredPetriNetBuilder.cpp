...
namespace PetriEngine {
    ...
    : _placenames(orig._placenames), _transitionnames(orig._transitionnames),
       _places(orig._places), _transitions(orig._transitions),
        _stable(*this, orig._stable),
        _fixed_point(*this, orig._fixed_point),
        _symmetries(*this, orig._symmetries)
    {
    }
    ...
    void ColoredPetriNetBuilder::unfoldTransition(uint32_t transitionId) {
        ...
        if(_fixed_point.computed() || _partitionComputed){
            FixpointBindingGenerator gen(transition, _colors, _symmetries.symmetries()[transitionId], _fixed_point.variable_map()[transitionId]);
            size_t i = 0;
