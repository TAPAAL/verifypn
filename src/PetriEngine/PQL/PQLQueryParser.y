		...
		| FIREABLE LPAREN id_list RPAREN
		    {
		        $$ = nullptr;
		        {
		            auto ids = $3;
		            if ((*ids).size() == 1) {
		                $$ = new FireableCondition(std::make_shared<std::string>((*ids)[0]));
		            } else {
		                std::vector<Condition_ptr> a;
                        for (auto& name : *ids) {
                            a.push_back(std::make_shared<FireableCondition>(std::make_shared<std::string>(name)));
                        }
                        $$ = new OrCondition(a);
		            }
		        }
		    }
		| ID QUESTIONMARK           { $$ = new FireableCondition(std::make_shared<std::string>(*$1)); }
		;
...
factor	: LPAREN expr RPAREN	{ $$ = $2; }
		...
		| ID			{ $$ = new IdentifierExpr(std::make_shared<std::string>(*$1)); delete $1; }
        | TOKENCOUNT LPAREN id_list RPAREN
            {
                $$ = nullptr;
                {
                    ...
                    for (auto& name : *ids) {
                        a.push_back(std::make_shared<IdentifierExpr>(std::make_shared<std::string>(name)));
                    }
